...
export async function createNewChat(participantIds: string[]) {
  try {
...
export async function getMessagesByChatId(chatId?: string) {
  ...
  try {
    const comments = await databases.listDocuments(
      ...
      [Query.equal("room", chatId), Query.orderDesc("$createdAt")]
    );
...
// ============================== CREATE MESSAGES
export async function createMessages(message: IMessage) {
  try {
    ...
    if (!newMessage) throw Error("Failed to create comment");
